...
fourier.prototype.transformFFT = function () {
    ...
    let data = FFTImageDataRGBA(imgData.data, preview.height, preview.width)

    for(let x=0; x<preview.height/2; x++){
        for(let y=0; y<preview.width/2; y++){
            let pos = ((x * preview.width) + y) * 4;
            let shift = (((x + parseInt(preview.height/2)) * preview.width) + (y + parseInt(preview.width/2))) * 4
            imgData.data[pos] = data.real[shift]
            imgData.data[pos+1] = data.real[shift+1]
            imgData.data[pos+2] = data.real[shift+2]
        }
    }
    for(let x=parseInt(preview.height/2); x<preview.height; x++){
        for(let y=0; y<preview.width/2; y++){
            let pos = ((x * preview.width) + y) * 4;
            let shift = (((x - parseInt(preview.height/2)) * preview.width) + (y + parseInt(preview.width/2))) * 4
            imgData.data[pos] = data.real[shift]
    ...
    for(let x=0; x<preview.height/2; x++){
        for(let y=parseInt(preview.width/2); y<preview.width; y++){
            let pos = ((x * preview.width) + y) * 4;
            let shift = (((x + parseInt(preview.height/2)) * preview.width) + (y - parseInt(preview.width/2))) * 4
            imgData.data[pos] = data.real[shift]
            imgData.data[pos+1] = data.real[shift+1]
